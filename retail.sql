...
ALTER TABLE order_items CHANGE COLUMN `ï»¿order_id` `order_id` INT;

-- Inner Join for Order Details
SELECT o.order_id,o.order_date,oi.item_id,oi.product_id,
    p.product_name,oi.quantity,oi.list_price,oi.discount,oi.total_price
FROM orders o
...
-- Stock Alert Query
DESCRIBE stock;
ALTER TABLE stock CHANGE COLUMN `ï»¿store_id` store_id INT;

SELECT p.product_id, p.product_name, p.brand_id, p.category_id, 
    s.store_id, s.quantity
FROM products p
